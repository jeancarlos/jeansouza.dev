...
describe('initStar', () => {
  ...
  it('creates star with zero glitch timer', () => {
    const star = initStar(800, 600)
    expect(star.glitchTimer).toBe(0)
  })
})

describe('projectStar', () => {
  it('returns screen position for a centered star', () => {
    const star = { x: 0, y: 0, z: FOCAL_LENGTH, glitchTimer: 0, glitchChar: '' }
  ...
  it('returns larger size for closer stars', () => {
    const far = { x: 0, y: 0, z: 100, glitchTimer: 0, glitchChar: '' }
    const near = { x: 0, y: 0, z: 10, glitchTimer: 0, glitchChar: '' }
